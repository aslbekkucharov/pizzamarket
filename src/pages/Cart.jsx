...
function Cart() {
    ...
    const onRemoveCartItem = (arg) => {
        dispatch(removeCartItem(arg));
    };
    ...
    const cartPizzas = pizzas.map((data, index) => {
        return <CartItem onMinusItem={onMinusCartItem} onPlusItem={onPlusCartItem} removeCartItem={onRemoveCartItem} key={`${data.id}_${index}`} data={data} pizzaTotalPrice={items[data.id].totalPrice} pizzaTotalCount={items[data.id].items.length} />;
    });
